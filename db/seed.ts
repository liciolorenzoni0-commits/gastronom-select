...
async function seed() {
  ...
  const candidate1Data: InsertCandidate = {
    ...
    role: "sous_chef",
    experienceYears: 8,
    tags: JSON.stringify(["Michelin Exp", "Plating", "French", "Pastry", "Leadership"]),
  ...
  const candidate2Data: InsertCandidate = {
    ...
    role: "chef",
    experienceYears: 15,
    tags: JSON.stringify(["Executive", "Asian Fusion", "Michelin Star", "Menu Design", "Team Building"]),
  ...
  const candidate3Data: InsertCandidate = {
    ...
    role: "chef",
    experienceYears: 6,
    tags: JSON.stringify(["Pastry", "Chocolate Work", "Sugar Art", "Fine Dining", "Creative"]),
  ...
  const aiData: InsertAiSummary = {
    ...
    strengths: JSON.stringify(["Exceptional technical skills — knife work, sauces, and plating all scored above 9.5", "Michelin-level experience with understanding of fine dining standards", "Strong leadership track record with team management experience", "Exemplary kitchen hygiene practices", "Creative plating that balances artistry with functionality"]),
    concerns: JSON.stringify(["Speed during peak service could improve — scored 8.0", "May need support transitioning to our specific kitchen workflow"]),
